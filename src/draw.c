...
void sigwinchHandler(int sig) { sigwin = true; }
...
static void window_resize(term_t *t) {
    ...
    if (t->MAX_COL == w.ws_col && t->MAX_ROW == w.ws_row)
        return;
    ...
    if (t->buffer_size < w.ws_col * w.ws_row) {
        if (t->pixels != NULL) {
            free(t->pixels);
            t->pixels = NULL; // Set pointer to NULL after freeing to prevent
                              // double free
        }
        ...
        if (t->buffer != NULL) {
            free(t->buffer);
            t->buffer = NULL; // Set pointer to NULL after freeing to prevent
                              // double free
        }
        ...
        t->buffer = (char *)malloc(sizeof(char) * size *
